program swap
  ...
  real (c_double), allocatable :: pair_gaps(:), near_gaps(:), log_gap(:), log_pair_gaps(:), log_near_gaps(:)
...
!  call printmat(dim, H, 'R')
  ...
  do iteration = 1, n_iterations
    ...
    call dpquicksort(E)
    !print *, E(:)
    call log_gap_difference(dim_Sz0, E, log_near_gaps, log_pair_gaps, log_gap)
