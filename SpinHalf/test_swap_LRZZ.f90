program test_LR
  ...
  integer (c_int)     ::  i, j, l, k, p, q, dim_Sz0
  integer (c_int)     ::  unit_ph

  real(c_double), dimension(:), allocatable :: Jxy, hz
  real(c_double), dimension(:,:), allocatable :: Vzz
  real(c_double) :: T0, T1, Jxy_coupling, Vzz_coupling, hz_coupling, kick, alpha
...
!  call printmat(dim, H, 'R')
  ...
  !$OMP DO private(iteration, hz, Vzz, H, E, W_r, &
  !$OMP & U, W, PH)
