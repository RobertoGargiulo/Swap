program test_LR
  ...
  write(filestring,93) "data/eigen/quasienergies_Swap_LR_PBC_Sz0_nspin", &
...
!  call printmat(dim, H, 'R')
  ...
  do i = 1, n_disorder
    ...
    do k = 1, nspin-1
      Vzz(k,1:k) = 0
      do q = k+1, nspin
        Vzz(k,q) = Vzz(k,q) / ( min(abs(k-q), nspin - abs(k-q)) **alpha )
      enddo
      !write (*,*) Vzz(k,:)
    enddo
...
subroutine write_info(unit_file)
  ...
  write (unit_file,*) "V = V_{ij}/|i-j|^alpha, with V_{ij} is taken in [-3V/2, -V/2] with PBC; &
