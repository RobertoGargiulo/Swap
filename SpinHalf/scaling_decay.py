...
for i in range(num_J):
    for j in range(num_alpha):
        for q in range(numL):
            L = Larray[q]
            print([J[i], alpha[j], L])
            tau[q, i, j] = data[q, i, j][:, 1] - n_periods[q,i,j] + 1 
            #This addition is necessary because of the way 'decay' counts time
...
for q in range(numL):
    ...
    for i in range(num_J):
        for j in range(num_alpha):
            ...
            print(L, J[i], alpha[j], tau_avg[q,i,j], tau_err[q,i,j], n_periods[q,i,j])
...
for i in range(num_J):
    for j in range(num_alpha):
        ...
        for q in range(numL):
            ...
            #plt.hist(X[q, i, j], linewidth=1.2, label=string, alpha=0.5)
